...
s_pib <- df2021_clean %>% group_by(df2021_clean$Continent) %>% summarise(
  Médiane = median(`gdp`, na.rm = TRUE),
  Moyenne = mean(`gdp`, na.rm = TRUE),
  "Écart type" = sd(`gdp`, na.rm = TRUE),
  Minimum = min(`gdp`, na.rm = TRUE),
  Maximum = max(`gdp`, na.rm = TRUE)
)
...
**Données sur le PIB estimé, moyenne par continent, 2021**
```{r}
s_pib %>%
  kbl() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F)
