...
d'un mois, on obtient des observations dont la date varie entre le 
`r min(new$date)` et le `r max(new$date)`, avec le `r mean(new$date)` comme date 
moyenne. En d'autre mots, l'écart varie de `r min(new$difference)` à 
`r max(new$difference)` jours avec une moyenne de `r mean(new$difference)`. 
Visuellement, voici la distribution : 
...
```{r, include=FALSE}
summary(df_pays$date)

```



###Les plus récentes données de la vaccination

*une analyse de la variable date (min, max,)*
*graph de la distribution des dates*

### Les données sur la vaccination
